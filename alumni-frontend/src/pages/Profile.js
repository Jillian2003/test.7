...
const USERS_URL = "http://104.131.98.186:2490/api/users";
const MAJORS_URL = "http://104.131.98.186:2490/api/majors";
...
function Profile() {
  ...
  const majorDetails = useMemo(() => {
    ...
    const wanted = String(profile.major).trim().toLowerCase();
    return (
      majors.find(m => String(m?.name ?? "").trim().toLowerCase() === wanted) || null
    );
  }, [profile, majors]);
